...
fs.readdir('./www/js', {encoding: 'utf8'}, (err, files) => {
  if (err) {
    console.log("Error reading partials dir:".red, err);
    return;
  }
  ...
  for (let file of files) {
    fs.readFile('./www/js/'+file, 'utf8', (err, data) => {
      if (err) {
        console.log(`Error reading partial ${file}:`.red, err);
        return;
      }
...
console.log("Skip weeks:".yellow, skipWeeks);
  ...
  _.each(slot, series => {
    if (fs.existsSync(`series/${series.picture}.png`)) {
      fs.copyFile(`series/${series.picture}.png`, `../dist/images/series/${series.picture}.png`, (err) => {
        if (err) {
          console.log("Error copying series picture:".red, err);
        }
      });
    }
  });
...
_.each(basData.events, (event) => {
  if (event.picture !== undefined && fs.existsSync('series/'+event.picture+'.png')) {
    fs.copyFile(`series/${event.picture}.png`, `../dist/images/series/${event.picture}.png`, (err) => {
      if (err) {
        console.log("Error copying event picture:".red, err);
      }
    });
  }
});
...
console.log("Today:           ", util.formatShortDate(now).yellow);
...
_.each(basData.top10, person => {
  _.each(person.anime, series => {
    ...
    if (fs.existsSync(`series/${series.picture}.png`)) {
      fs.copyFile(`series/${series.picture}.png`, `../dist/images/series/${series.picture}.png`, (err) => {
        if (err) {
          console.log("Error copying top 10 series picture:".red, err);
        }
      });
    }
  });
...
setTimeout(() => {
  ...
  sass.render({
    ...
    if (err) {
      console.log("Error writing stylesheet:".red, err);
    }
